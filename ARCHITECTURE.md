...
| `Tabs` + `Tab` | 48dp, 3dp indicator; 64dp when any tab has an `icon:` (24dp, stacked above the label -- every tab in the bar shares the taller height, `TabsElement` stretches icon-less siblings to match via `CrossAxisAlignment.STRETCH`) | primary rounds the indicator, secondary is flat |
