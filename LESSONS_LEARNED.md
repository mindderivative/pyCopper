...
A working record of what pyCopper's development has taught us: bugs and their
...
| **Flutter-style constraint layout** | Flexbox, Cassowary (constraint solver) | See `ARCHITECTURE.md` and the graph entity `pyCopper Architecture` for the full comparison. |
...
| **Logical DIPs**, converted to physical pixels exactly once, in paint | Converting at multiple layers | Makes M3's `dp` figures map 1:1 into pyCopper's `width`/`height`/`font_size`. |

Full reasoning for each: `ARCHITECTURE.md` (repo authority) and the graph
entity `pyCopper Architecture`.

**Naming/versioning decisions:**
- `pycopper` is a **phase codename**, not the framework's final identity — a
...
- `rendercanvas`'s GLFW backend submits `{"data": ..., "char_str": ...}` for
  ...
  `src/pycopper/app.py`'s `_on_canvas_event` (`event.get("data", "")`);
...
2. It also claimed *"pyCopper has no seam at which to hold a stale
...
real Wayland platform limitation, not a pyCopper bug) and switch technique
...
`pycopper-widget-backlog.md` (resize saga), `ARCHITECTURE.md` §5.8.1.
...
Full numbers: Claude memory `pycopper-performance-findings.md`, graph
entity `pyCopper Performance Findings`.
...
- **`materialyoucolor` is used as a class, not instantiated**; its
  attributes are camelCase while pyCopper's token names are snake_case.
...
Full detail: Claude memory `pycopper-tech-stack-traps.md`, graph entity
`pyCopper Tech Stack`.
...
`pyCopper Text Rendering Quality` — don't re-derive from scratch):
...
- **`Row`/`Column` were renamed to `Horizontal`/`Vertical`, repo-wide
  ...
  `pycopper.layout.algorithms` layer. Reason: their spreadsheet meaning
...
Full detail: Claude memory `pycopper-engineering-traps.md`, graph entity
`pyCopper Engineering Traps`; the September 2026 full-codebase review
(`docs/CODE_REVIEW_2026-09.md`, commits `13da50f`..`ccb13b1`, 1788 tests
passing) is the single largest source of these — see also
`pycopper-code-review-2026-09.md`.
...
`pycopper-widget-backlog.md`, `pycopper-widget-review-backlog.md`,
`pycopper-engineering-traps.md`, `pycopper-code-review-2026-09.md`,
`pycopper-naming.md`, `pycopper-performance-findings.md`,
`pycopper-tech-stack-traps.md`, `pycopper-text-rendering.md`,
`pycopper-font-bundle.md`, `m3-references-library.md`,
`pycopper-skills.md`, `feedback-verify-empirically.md`,
...
`pyCopper Architecture`, `pyCopper Milestone History`, `pyCopper Widget
Backlog`, `pyCopper Package Survey`, `pyCopper Rendering Engine Evaluation`,
`pyCopper Engineering Traps`, `pyCopper Performance Findings`, `pyCopper
Tech Stack`, `pyCopper Text Rendering Quality`, `pyCopper Font Bundle`,
`pyCopper Slider Design Backlog`, `pyCopper ButtonGroup Design Backlog`,
`pyCopper Overlay Demo YAML Bug`.
